...
public class MainWeatherFrame extends javax.swing.JFrame {
    ...
    public MainWeatherFrame(FeedEntry entry) {
        ...
        InitTimer();
        Init();
        LocationLabel.setText(entry.getLocation());
        pack();
        setLocationRelativeTo(null);
        setVisible(true);
    }
    
    public void setLocationBasedOnString(String locationString) {
            entry.setLocation(locationString);
            entry.setCountryCode("null");
            LocationLabel.setText(entry.getLocation());
    }
    ...
    private File getResourceAsFile(String resourcePath) {
     ...
     private void initComponents() {
        ...
        CurrentTempLabel.setFont(new Font("Segoe UI", 0, 46)); // NOI18N
        ...
        // Add the "Weather Forecast in" label
        jLabel45.setFont(new Font("Segoe UI", 0, 30)); // Set font
        jLabel45.setForeground(new Color(255, 255, 255)); // Set text color
        jLabel45.setText("Weather Forecast in ");
        jPanel2.add(jLabel45, new AbsoluteConstraints(30, 5, -1, 80));

        // Add the search field below the label
        searchField = new JTextField();
        searchField.setBounds(180, 130, 250, 20); // Adjust y-coordinate to place it below the label
        jPanel2.add(searchField, new AbsoluteConstraints(100,70, -1, -1));
        
        // Initialize and add the JComboBox in your UI setup method
        historyComboBox = new JComboBox<>();
        historyComboBox.setBounds(10, 130, 150, 20); // Adjust the position as needed
        jPanel2.add(historyComboBox, new AbsoluteConstraints(100, 90, -1, -1));
        ...
        @Override
            public void actionPerformed(ActionEvent e) {
                String selectedLocation = (String) historyComboBox.getSelectedItem();
                if (selectedLocation != null && !selectedLocation.isEmpty()) {
                    searchField.setText(selectedLocation);
                    setLocationBasedOnString(selectedLocation);
                    RefreshIconLabelMouseClicked(null);
                }
            }
        });
        
        // Create the toggle button for unit conversion
        JToggleButton tempToggleButton = new JToggleButton("Show Fahrenheit");
        tempToggleButton.setBounds(10, 160, 150, 20); // Position it below the historyComboBox
        jPanel2.add(tempToggleButton, new AbsoluteConstraints(30, 340, -1, -1)); // Adjust the position as needed
        
        tempToggleButton.addActionListener(new ActionListener() {
           @Override
           public void actionPerformed(ActionEvent e) {
               String currentText = CurrentTempLabel.getText();
               // Extract the numeric part of the temperature
               double temp = Double.parseDouble(currentText.split(" ")[0]);
               if (tempToggleButton.isSelected()) {
                   // Convert to Fahrenheit and update the button text
                   temp = temp * 9 / 5 + 32;
                   int tempInt = (int) temp; // Cast to int to remove decimal part
                   CurrentTempLabel.setText(tempInt + " °F"); // Display as integer
                   tempToggleButton.setText("Show Celsius");
               } else {
                   // Convert to Celsius and update the button text
                   temp = (temp - 32) * 5 / 9;
                   int tempInt = (int) temp; // Cast to int to remove decimal part
                   CurrentTempLabel.setText(tempInt + " °C"); // Display as integer
                   tempToggleButton.setText("Show Fahrenheit");
               }
           }
       });
        ...
        // Add the search button next to the search field
        searchButton = new JButton("Search");
        searchButton.setBounds(280, 100, 100, 20); // Adjust x-coordinate to position it next to the search field
        jPanel2.add(searchButton,new AbsoluteConstraints(180, 70, 100, 20));
        
        // Add action listener to the search button
        searchButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                String locationString = searchField.getText();
                addLocationToHistory(locationString);
                updateHistoryComboBox();
                setLocationBasedOnString(locationString);
                RefreshIconLabelMouseClicked(null);
            }
        });
        ...
        // Create the toggle button for wind speed unit conversion
        JToggleButton windToggleButton = new JToggleButton("Show km/h");
        windToggleButton.setBounds(320, 240, 120, 20); // Position it to the right of WindLabel
        jPanel2.add(windToggleButton, new AbsoluteConstraints(70, 270, 120, 20));
        
        windToggleButton.addActionListener(new ActionListener() {
        @Override
        public void actionPerformed(ActionEvent e) {
            String currentText = WindLabel.getText();
            // Extract the numeric part of the wind speed
            double windSpeed = Double.parseDouble(currentText.replaceAll("[^0-9.]", ""));
            if (windToggleButton.isSelected()) {
                // Convert m/s to km/h and update the button text
                windSpeed = windSpeed * 3.6; // 1 m/s = 3.6 km/h
                WindLabel.setText(String.format("Wind Speed: %.2f km/h", windSpeed));
                windToggleButton.setText("Show m/s");
            } else {
                // Convert km/h to m/s and update the button text
                windSpeed = windSpeed / 3.6; // Convert back to m/s
                WindLabel.setText(String.format("Wind Speed: %.2f m/s", windSpeed));
                windToggleButton.setText("Show km/h");
            }
            }
        });
        ...
        jPanel2.add(LocationLabel, new AbsoluteConstraints(450, 50, 200, -1));
        ...
        // Define image paths for different times of the day
        String morningImage = "/morning_background.jpg";
        String afternoonImage = "/afternoon_background.jpg";
        String eveningImage = "/evening_background.jpg";
        String nightImage = "/night_background.jpg";

        // Set default background image
        String backgroundImage = "";

        // Choose background image based on the time of the day
        if (hour >= 6 && hour < 12) {
        ...
        jLabel6.setIcon(new ImageIcon(getClass().getResource(backgroundImage)));
        jPanel2.add(jLabel6, new AbsoluteConstraints(0, 0, 990, 490));
    ...
    if (!searchHistory.contains(location.trim().toLowerCase())) {
        searchHistory.add(location.trim().toLowerCase());
        // Here you could add code to persist the list to a file or database
    }
    }
    
    // Method to update JComboBox items
    private void updateHistoryComboBox() {
        historyComboBox.removeAllItems();
        for (String location : searchHistory) {
            historyComboBox.addItem(location);
        }
    }
